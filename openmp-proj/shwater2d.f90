...
program shwater2d
  ...
  real(kind=dp), dimension(:,:,:), allocatable :: Q, Q_new
  real(kind=dp), dimension(:), allocatable :: x, y
  integer i, j, ifail, n, m
  ...
  !
  allocate(Q(cell_size, m, n), Q_new(cell_size, n, m), x(m), y(n), stat = ifail)
  ...
  !
  stime = rtc()
  call solver(Q, Q_new, cell_size, m, n, tend, dx, dy, dt, fx, fy)
  write(*,*) 'Solver took',rtc()-stime,'sec'

  !
  ! Uncomment this line if you want visualize the result in ParaView
  !
   !call save_vtk(Q, x, y, cell_size, m, n, '/scratch/knyl/result.vtk')
...
subroutine solver(Q, Q_new, l, m, n, tend, dx, dy, dt, fx, fy)
  use types
  implicit none
  ...
  real(kind=dp), intent(inout), dimension(l, n, m) :: Q_new
  ...
  do i=1,steps
     ...
     !
     call laxf_scheme_2d(Q, Q_new, l, m, n, dx, dy, dt, fx, fy)
     time = time + dt
...
subroutine laxf_scheme_2d(Q, Q_new, l, m, n, dx, dy, dt, fx, fy)
  use types
  implicit none 
  ...
  real(kind=dp), intent(in) :: dx, dy, dt
  real(kind=dp) ::  dtdx, dxdt, dtdy, dydt
  external fx, fy
  integer i,j
  ...
  !$omp parallel private(i,j, ffy, ffx, nFx, nFy) shared(Q, Q_new)
  !$omp do
  do i=2,n
     call fx(Q(:,:,i), ffx, l, m)
     do j=2,m
     ...
     do j=2,m-1
        Q_new(:,i,j) = Q(:,j,i) -  dtdx * ((nFx(:,j+1)  -  nFx(:,j)))
     end do
  end do
...
!$omp do
  do i=2,m
     call fy(Q_new(:,:,i), ffy, l, n)
     do j=2,n
        nFy(:,j) = 0.5  * ((ffy(:,j-1) + ffy(:,j)) - &
             dydt * (Q_new(:,j,i) - Q_new(:,j-1,i)))
     end do
     do j=2,n-1
        Q(:,i,j) = Q_new(:,j,i) -  dtdy * ((nFy(:,j+1)  -  nFy(:,j)))
     end do     
  end do
  !$omp end parallel
